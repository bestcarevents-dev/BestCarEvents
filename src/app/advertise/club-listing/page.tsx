...
export default function ClubListingPage() {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      const clubsData = await fetchByUser("clubs");
      setClubs(clubsData);
      setLoading(false);
    };
    fetchAll();
  ...
  const handleDeactivate = async (col: string, id: string) => {
    ...
    if (currentUser) {
      ...
      setClubs(await fetchByUser("clubs"));
    }
  };
  ...
  const activeListingsCount = clubs.filter((cl) => !cl.deactivated).length;
  const activeAdsCount = clubs.filter((cl) => cl.featured && !cl.deactivated).length;
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-8">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-4xl font-extrabold font-headline">Club Listing</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
        <Card className="w-full">
          <CardHeader>
            ...
            <CardDescription>All your club listings currently live.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="w-full">
          <CardHeader>
            ...
            <CardDescription>All your featured (advertised) club listings.</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {loading ? (
        <div className="text-center py-12 text-lg animate-pulse">Loading your club listings...</div>
      ) : (
        <Card className="mb-8">
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <CardTitle className="text-lg">Club Listing</CardTitle>
                <CardDescription>Manage your club listing here.</CardDescription>
              </div>
                    ...
                    <CardHeader className="pb-3">
                      <div className="flex items-start justify-between">
                        <div className="flex-1 min-w-0">
                          <CardTitle className="text-base truncate">{club.clubName || 'Unnamed Club'}</CardTitle>
                          <CardDescription className="truncate">{club.location || 'Location TBD'}</CardDescription>
                        </div>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                              onClick={() => handleDeactivate('clubs', club.id)}
                              disabled={club.deactivated}
                            >
                              Deactivate
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                    ...
                    <CardContent className="pt-0">
                      <div className="space-y-3">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="truncate">{club.location || 'TBD'}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <Badge variant={club.featured ? 'default' : 'outline'}>
                            {club.featured ? 'Yes' : 'No'}
                          </Badge>
                        </div>
                      ...
                      <div className="mt-4">
                        <Dialog open={advertiseModal?.open && advertiseModal.col === 'clubs' && advertiseModal.id === club.id} onOpenChange={(open) => {
                          if (!open) {
                            setAdvertiseModal(null);
                            setSelectedFeatureType(null);
                          }
                          ...
                          <DialogTrigger asChild>
                            <Button
                              variant={club.featured ? "secondary" : "default"}
                              size="sm"
                              disabled={club.featured || club.deactivated}
                              className="w-full"
                              onClick={() => {
                                setAdvertiseModal({ open: true, col: 'clubs', id: club.id });
                                setSelectedFeatureType(null);
                              }}
                            >
                              {club.featured ? "Featured" : "Feature"}
                            </Button>
                          </DialogTrigger>
                          <DialogContent className="max-w-md">
                            <DialogHeader>
                            ...
                            <div className="py-4 text-center">
                              <Button
                                ...
                                onClick={() => handleAdvertise('clubs', club.id)}
                              >
