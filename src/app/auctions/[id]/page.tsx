    ...
    <div className="container mx-auto px-4 py-10 bg-white animate-fade-in">
      ...
      <div className="relative w-full max-w-5xl mx-auto rounded-2xl overflow-hidden shadow-2xl aspect-video mb-6 group">
        ...
        <button className="absolute inset-0" aria-label="Open image gallery" onClick={() => lightbox.open(images, carouselIdx)} type="button">
          <Image src={images[carouselIdx] || "https://via.placeholder.com/900x500?text=No+Image"} alt={auction.auctionName} fill className="object-contain w-full h-full" unoptimized />
        </button>
      </div>
        ...
        <div className="max-w-5xl mx-auto mb-8 flex gap-3 overflow-x-auto pb-2">
          {images.map((img, idx) => (
            <button key={idx} className={`relative w-36 h-24 rounded-lg overflow-hidden border-2 ${carouselIdx === idx ? 'border-yellow-600' : 'border-transparent'}`} onClick={() => setCarouselIdx(idx)} aria-label={`Go to image ${idx + 1}`}>
              <Image src={img} alt={`Auction image ${idx + 1}`} fill className="object-contain" unoptimized />
            </button>
          ))}
        </div>
      )}
          ...
          <Card className="p-6 bg-white border border-gray-200">
            <CardContent className="p-0">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-gray-900">
                <div className="flex items-center gap-2"><Calendar className="w-4 h-4 text-yellow-600" /><span className="font-semibold">Start Date:</span> <span>{formatDate(auction.startDate)}</span></div>
